        ...
        <v-col v-for="(item, index) in files" :key="index" cols="2">
          <v-hover v-slot:default="{ hover }">
            <v-card class="pa-2 file-card" :class="{'active': checkFileSelected(item)}" ref="files"
              ...
              <v-card-text>
                <v-img :src="require('~/assets/image/pdf.png')" alt=""></v-img>
              </v-card-text>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        files: state => state.document.files,
    ...
    methods: {
      ...mapActions({
        ...
        removeFileSelected: 'document/removeFileSelected',
        addFileSelected: 'document/addFileSelected',
